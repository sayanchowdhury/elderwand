...
if yesno "Is this correct?"; then
    ...
    git flow release finish -m $new_vers -u 9CD05C8B $new_vers
else
    echo Okay.. bailing.
    exit 1
fi
...
if yesno "Mail that changelog to the infrastructure list?"; then
    echo "$changelog" | vipe | mutt -e "source ~/.mutt/gmail" -s "[release] $project: $new_vers" infrastructure@lists.fedoraproject.org
fi
